...
class HomeRepository {
  ...
  String url = 'http://172.16.0.192:8082/api/v1/homes';
  String key =
  ...
  Future getHomes() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Home> home =  body.map((dynamic item) => Home.fromJson(item)).toList();
      return home;
    } else {
      throw Exception('Failed to load home');
    }
  }
  Future createHome(String verHard, String verSoft,dynamic dateFab,
  dynamic dateOeuvre,  dynamic zones,  String label, String location) async {
  ...
  Future updateHome(String idShFab, String verHard, String verSoft,dynamic dateFab,
      dynamic dateOeuvre,  dynamic zones,  String label, String location) async {
    final msg = jsonEncode({
      'verHard': verHard,
      'verSoft': verSoft,
      ...
      'label': label,
      'location': location });
    final response = await http.put(Uri.parse(url + '/$idShFab'),
        headers: {
          "Authorization": key,
          "Accept": "application/json",
          "content-type": "application/json"
  ...
  Future deleteHome(String idShFab) async {
    final response = await http.delete(Uri.parse(url + '/$idShFab'), headers: {
      "Authorization": key,
      "Accept": "application/json",
      "content-type": "application/json"
    });
