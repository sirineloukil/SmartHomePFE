...
class ZoneRepository {
  String url = 'http://172.16.0.192:8082/api/v1/zones';
  String key =
  ...
  Future getZones() async {
    ...
    if (response.statusCode == 200) {
      ...
      List<Zone> zone =
          body.map((dynamic item) => Zone.fromJson(item)).toList();
      return zone;
    } else {
  ...
  Future createZone(String label, dynamic devices, dynamic pairings) async {
  ...
  Future updateZone(
      String idZ, String label, dynamic devices, dynamic pairings) async {
    final msg = jsonEncode({
      'label': label,
      'devices': devices,
      'pairings': pairings,
    });
    final response = await http.put(Uri.parse(url + '/$idZ'),
        headers: {
          "Authorization": key,
          "Accept": "application/json",
          "content-type": "application/json"
  ...
  Future deleteZone(String idZ) async {
    final response = await http.delete(Uri.parse(url + '/$idZ'), headers: {
      "Authorization": key,
      "Accept": "application/json",
      "content-type": "application/json"
    });
